...
- **Offer Content property editor:** `contentUrl` draft, `contentText` draft, active-field identity, and local validation/error state are transient authoring state owned by `MapSpike.tsx`. Only committed `contentUrl` and `contentText` belong to the Offer in `MapDocument`.
- **Offer Content read presentation:** expanded/collapsed Offer identity, the disclosure-control viewport snapshot, and its scroll-owner/context are transient read-state presentation owned by `MapSpike.tsx`. They exist independently of whether the property editor is open and do not mutate `MapDocument`.
...
| Offer Content property editor | `offerContentSection()` draft, active-field, and local error state/refs in `MapSpike.tsx` | Activate Add/Edit Offer Content | Two independent URL/text fields with recoverable URL error | Natural completion of one valid active field calls `updateOfferContent()` once | Close/outside/switch completes the valid active field first; invalid URL blocks dismissal; Escape abandons only that active draft | Drafts, active field, and local validation/error are transient authoring state; committed values are in-memory `MapDocument` state |
| Offer Content read presentation | `expandedOfferContentId`, `offerContentDisclosureRef`, and `offerContentDisclosureSnapshotRef` in `MapSpike.tsx` | Activate Show more/Show less | Content expands or collapses; eligible collapse restores the disclosure control's prior viewport offset in the same scroll context | **None** | Independent of property-editor dismissal; snapshot is cleared or consumed when its Offer/text/control/scroll context is no longer eligible | Transient read-state presentation; never `MapDocument` state |
...
- Offer-wide legacy **Apply** does not participate in Content editing. Separately owned read-state disclosure and collapse scroll restoration remain presentation interactions with commit effect **None**, whether or not the property editor is open.
...
**Duplicate current Offer.** Canonical `duplicateEntity()` defines Offer duplication. The duplicate receives the same Product and canonical Offer-owned authored state, including `contentUrl`, `contentText`, current Offer Job selections/addressed scope, and Offer Financial intent. It does not receive Connected Touchpoints, derived Neighborhood, or Touchpoint-owned encounter Job/Financial Desired Outcome selections. Neither transient Content authoring drafts nor the separately owned read-state disclosure/scroll presentation are copied. `duplicateOfferAndReplace()` composes canonical duplication with Touchpoint-owned replacement rather than implementing a forked copy algorithm. A collision-safe, distinguishable title is part of the accepted duplicate flow.
