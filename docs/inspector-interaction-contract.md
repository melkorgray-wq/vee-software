...
7. A mandatory relation cannot pass through an empty committed state. When removing its sole current value requires another value, the outer editor opens a transient nested replacement step; choosing an existing candidate, authoring a blank sibling, or planning a canonical duplicate changes no document, and one explicit confirmation commits creation plus replacement atomically. A created replacement reuses the canonical entity creation or duplication owner and must not copy encounter-owned state. Back/Escape returns one level without mutation. The review is required even when downstream impact is empty, names the current owner, departing value, and planned replacement, and includes the owner's exact removal impact when it is not.

Touchpoint meaning is not portable: Offers are multi-select structural presentation relations with at least one Offer required. Offer-side removal of the sole linked Offer therefore uses the nested atomic replacement grammar and the Touchpoint-owned complete-set impact/commit chain, never Clear or detach-then-attach. Parent is optional single-select; Children attach/detach/reassign/create use dedicated commands and may require ancestor contributor resolution. `deriveTouchpointChildrenCandidates`, `deriveTouchpointReassignTargets`, `commitTouchpointParent`, and `planTouchpointStructuralChange` own those semantics. A generic radio/checkbox does not authorize generic setters, direct edges, containment, or candidate eligibility.
