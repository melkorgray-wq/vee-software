...
The hardened Touchpoint Inspector currently demonstrates heading-based edit affordances, embedded immediate-commit relation editing, structural Children authoring, derived/read-only Neighborhood, grouped Client scope read panels, and Inspector history that survives a Map round trip without a changed Map selection. Product and Offer Inspectors still retain transitional section-wide Apply mechanics; these must not be copied into future Inspector kinds. The remaining hardening work is based on reproducible runtime defects, not an open-ended redesign.
...
A Product Job Intent for a DO-bearing Job may have an empty Desired Outcome subset when the user knows the Job is relevant but has not yet described the relevant outcomes. Product, Offer, and Touchpoint each own independent local Job membership and an independently mutable Desired Outcome subset. Membership requires a selection record; a Desired Outcome requires membership, while membership never requires a Desired Outcome. This intent knowledge must not be interpreted as permission to bypass the Desired Outcome layer in a materialized route.

Entity Inspector editing follows semantic completeness rather than a blanket save-on-control-change or blanket draft-and-Apply rule. A semantically complete, unambiguous, valid, non-destructive action commits automatically as exactly one owner-level domain transaction. A checkbox click is the commit trigger when the checkbox itself expresses that complete semantic operation. Title and other property editors commit on their natural completion, such as Enter or blur, and relation selection commits as soon as every required semantic choice and contributor is known. Closing a transient picker or editor is an independent UI-state transition, not a commit boundary.

When a displayed Inspector value already has its own action, the value and its edit affordance are distinct semantic surfaces. In read state, an authorable property uses an interactive property heading with the same `Click to edit` hint on hover/focus; a permanently visible pencil is legacy, not the portable edit affordance. The heading opens the embedded local editor, while linked entity titles navigate internally and URL values navigate externally. Title uses direct inline editing. Where a value has no independent navigation action (currently the catalogue-only `Located in` label), its heading and value may both open the same editor; a linked Parent value remains navigation, whereas its heading opens editing. An empty `Add parent` or `Add URL` may open the corresponding editor. A single hit area must not conceal a combination of navigation and editing.

In read state, an entity title may itself be a navigation surface. In a local selection or edit state, a title that identifies the value being selected belongs to that selection control: one compact row must not interpret its primary title as both selection and navigation away. Deliberately separate secondary navigation remains available when it is visually and semantically distinct from the selection action.

Relation editors are embedded in their local Inspector property, and each candidate row is one complete action. Multi-select relations use checkbox-like rows; single-select relations use radio-like rows whose candidates represent only real, mutually exclusive relation targets. Clearing an optional single relation is an explicit local action rather than a synthetic candidate, and a complete, valid, non-destructive clear operation commits immediately. Complete operations commit immediately: a single-select editor closes after selection, while a multi-select editor remains open until explicit **Close**. Close, Escape, outside interaction, or opening another local editor dismisses transient UI without rolling back committed changes. Search is disclosed only when the candidate space is large enough to need it. **Cancel** is reserved for an incomplete operation or one awaiting confirmation, rather than ordinary relation-editor dismissal.

The portable Inspector UI distinguishes entity navigation, authoring actions, secondary state/metadata, and transient controls in appearance as well as behavior. Entity navigation uses a consistent text-link treatment, not an action button or a status chip. `Close` is a compact secondary dismiss action outside candidate rows; `Back` returns one level within a nested editor flow without undoing completed independent commits; `Cancel` discards only a genuinely unfinished operation. Search and contributor resolution scale with the relation's candidate space; a simple Business-side picker need not copy the full Client-intent discovery UI. Structural and derived section headings remain distinguishable from interactive property headings. These are transferable interaction invariants, not a claim that every Inspector kind already implements the hardened Touchpoint reference.

Missing ontology, hierarchy, or contributor context pauses only the affected operation until that semantic choice is supplied; it does not justify staging unrelated fields or an entire Inspector section. Invalid operations remain correctable without mutation. Destructive operations pause only for a disclosed impact review and explicit confirmation, after which the complete change commits once. Validation or transaction failure changes no durable record and leaves a coherent interaction that can be corrected or retried. `Apply`, `Save`, `Done`, broad staged forms, and section-level completion actions are not the target interaction pattern when a change can be expressed as a complete local owner-level transaction.

### In-memory transaction history and explicit commit direction

The current spike has no undo journal and no external persistence. `MapDocument` changes become live in memory when an owner-level domain operation completes; incomplete, invalid, cancelled, or failed interaction state is not part of that document. Consequently an interface must not describe the current state as saved to disk, synchronized, backed up, or otherwise externally durable.

A transaction journal is **Proposed**, not implemented or accepted architecture. The proposed boundary records the complete `MapDocument` immediately before and after each successful owner-level domain transaction. One undo restores the complete prior document for exactly one such transaction, including domain records and authored placements changed by that operation. Selection, focus, camera, open panels, transient drafts, validation messages, and other UI state are not document history. After restoration, selection should remain on the same entity when it still exists and otherwise clear; camera and workspace view should remain unchanged. Editable controls retain native platform undo because application shortcuts must not intercept undo or redo while an input, textarea, select, or contenteditable control owns the event.

The proposed history is scoped to the current in-memory workspace session. Switching to a different `MapDocument` starts an independent empty history for that document; reload or process exit discards history. A new domain transaction after undo clears the redo branch. Redo is proposed alongside undo so an accidental undo is reversible, using `Ctrl/Cmd+Shift+Z`; `Ctrl+Y` remains an unresolved platform-alias question. A bounded entry count and byte budget must be selected from measurement before implementation; until then no production memory guarantee is claimed.

`Ctrl/Cmd+S` is also not implemented as persistence and must not become a secondary commit mechanism for an already-complete local edit. Complete local operations commit through their natural interaction trigger. If no operation is pending, the proposed command may state **“Changes are already live in this session.”** An incomplete or invalid operation remains uncommitted and receives its existing validation guidance; the shortcut neither supplies missing semantics nor inserts it into transaction history. It must not claim external persistence.

The owner, alternatives, costs, rollback, and migration implications for this proposal are recorded in ADR 0002. Implementation is deferred until that foundational boundary is reviewed and all durable `MapDocument` writes are routed through one transaction owner; storing incidental React component snapshots is not an acceptable substitute.

| State | Owner and trigger | Durable result | Visible interaction |
| --- | --- | --- | --- |
| `complete` | The active local operation has every required semantic input and contributor, validation succeeds, and no confirmation is required | Its checkbox click, natural property-edit completion, or final relation choice triggers one atomic owner-level domain commit automatically | The committed relationship or property is shown; no redundant section-level completion affordance is presented for that action |
| `incomplete` | A required semantic input is absent, including an unresolved DO-bearing scope or an unchosen contributor among multiple eligible Offers | None | The missing choice remains visibly pending and commit is unavailable |
| `invalid` | Catalogue, topology, contributor, or field validation rejects the proposed operation | None | The error is associated with the active operation and may be corrected |
| `destructive-awaiting-confirmation` | A Product/Offer removal, Product change, Touchpoint Offer unlink, or other operation has dependent durable impact | None | A consolidated impact review offers Cancel and explicit confirmation |
| `committing` | A complete operation has begun its single domain transaction | No partial intermediate document is observable | Repeated activation is suppressed while the operation is in flight |
| `failed` | Validation changed before commit or the domain transaction failed | The prior durable document is retained exactly | The error remains visible and the relevant choice or confirmed draft remains recoverable |

Product Client-intent branch construction, Product and Offer property or intent edits, and ordinary Touchpoint properties (Title, linked Offers, parent, Located in, URL, and mitigation) follow the same local semantic-completeness rule. A multi-control presentation alone does not combine those edits into one operation. A checkbox commits immediately when its change is a complete, valid, unambiguous, non-destructive semantic operation; otherwise only that operation waits for missing context, correction, or destructive-impact confirmation. A complete Touchpoint semantic-leaf addition with one linked Offer uses that unambiguous implicit contributor and commits immediately; with multiple linked Offers it remains incomplete only until the author chooses contributor(s), then the whole required upstream Product/Offer completion and attributed Touchpoint selection commit atomically. Touchpoint Offer unlink requires explicit destructive-impact confirmation whenever attributed paths are affected.

An authorable inverse relation does not become read-only because it is entered from its opposite side. **Children** are ordinary Touchpoints connected by containment, not a separate entity kind. Attach, detach, and reassign commit immediately once semantically complete; a move and its additive ancestry reconciliation are one atomic operation, while detach never prunes former ancestry. Missing contributor context delays only that structural operation, and a sole valid contributor is inferred rather than asked again. Embedded child creation likewise commits as soon as its title and one required initial Offer are known. Status feedback is reserved for committed consequences outside the currently visible structure, rather than restating a visible local change.

Product Inspector Client-intent authoring may hold a transient operation only while required semantic information is missing, validation prevents a valid operation, or destructive impact awaits confirmation. Product properties, selections, and Client entities commit through their own complete owner-level operations rather than an Inspector-wide action. An uncommitted Client entity exists only while it participates in the operation that creates or selects it; removing an uncommitted Job discards its uncommitted descendants, and removing an uncommitted Desired Outcome discards that outcome. Existing durable Client entities are never deleted by unselecting Product intent.

Disclosure of a DO-bearing Job is independent from Product-intent selection. Selecting a Desired Outcome always includes its owning Job, but selecting a Job never assumes any Desired Outcome, and removing the final selected Desired Outcome leaves a valid, incomplete upstream Product Job Intent. Core Functional Job, Related Job, and Consumption Chain Job use ordinary Desired Outcomes; Emotional and Social Jobs do not. An ordinary Desired Outcome remains owned by its Job through the Client-side `Job → Desired Outcome` relationship and never becomes a standalone Product goal. Creating one during intent authoring creates the Client-side entity and its ordinary owning relationship before selecting it inside Product Job Intent; “before” describes mutation order within the same atomic owner-level transaction, not an earlier durable commit. It creates no direct Product → Desired Outcome relationship.

The current runtime's Product and Offer Inspectors still construct and validate section-wide next documents through **Apply**, holding their properties and Client-intent selections—and, for Product, new Client entities—in drafts until then. This Inspector-wide Apply mechanic is transitional implementation debt, not accepted interaction direction and not a pattern for future Product, Offer, Touchpoint, Client-side, or Context Inspectors to copy. While that debt remains, failure commits nothing and preserves the recoverable draft; durable Product and Offer Job branches remain in the section established when the draft opened; temporarily deselecting a durable Product Job remembers its latest draft Desired Outcome scope for re-selection until Apply or Discard; and a guard protects a dirty Product draft when navigation would replace or abandon it. These are current-runtime facts, not target commit semantics. Removing or narrowing durable Product intent continues to require a consolidated in-app downstream-impact review before commit when Offer or Touchpoint selections would be removed or a Touchpoint Desired Outcome subset would be narrowed; additive changes require no such review.
...
An Offer selects Product Job Intents belonging to its Product and owns an explicit local subset of the Product subset, including a valid empty subset. The stable Offer selection identifies the `Offer + ProductJobIntent` path; changing its subset does not replace that identity. It does not create a direct Offer → Desired Outcome relationship. Separately, Offer owns explicit **Financial Desired Outcome intent**, independently of Product Job Intent. A new Offer may be structurally created beneath its Product before its Client intent is understood. Initial creation authors only the Offer and its required Product → Offer relationship: Product Client intent is not silently copied or initialized into the Offer, and Financial Desired Outcome intent is not a creation prerequisite. Offer Client intent and Financial Desired Outcome intent are authored afterward through Entity Inspector or another explicit semantic-authoring operation. Existing Offers likewise do not automatically acquire Product Job Intents authored later.

Offer Inspector uses the same Job → Desired Outcome mini-dendrite language as Product Inspector. A Job checkbox controls local Offer membership, while nested Desired Outcome checkboxes control the Offer-owned subset within Product-authored availability; an empty subset remains valid. Each checkbox is its operation's immediate commit trigger when the resulting selection is complete, valid, unambiguous, and non-destructive. Financial Desired Outcome intent remains a separate Offer-owned selection. Changing Linked Product does not initialize intent from the new Product. Destructive intent removal or Product change requires an in-app downstream-impact review when attributed Touchpoint paths would be removed; only that affected operation pauses for confirmation.
...
Touchpoint remains the only visible Business/Client encounter boundary. A Touchpoint authors a concrete selection from the intent available through its linked Offers rather than unconditionally inheriting the union of all linked Offer intent. Touchpoint Inspector uses semantic mini-dendrites from the existing Client ontology rather than a flattened or Touchpoint-specific ontology. Each semantic leaf selection records explicitly the contributing linked Offer path through which the selected ordinary Desired Outcome, Emotional or Social Job, or Financial Desired Outcome acts. A selection cannot outlive that contributing path. One semantic leaf may have multiple Offer paths, and different Desired Outcomes of the same Job may use different contributing Offers. Different Touchpoints of the same Offer may have overlapping, disjoint, or otherwise different Client-intent subsets.

For a DO-bearing Job, Touchpoint authoring may retain local Job membership with an empty Desired Outcome subset. That state is visible as intent knowledge in Inspector but creates no cross-side route. A non-empty subset is validated against the immediate contributing Offer and materializes routes only through those selected Desired Outcomes. The system must not create a direct fallback route around that missing layer.

When a Touchpoint has exactly one linked Offer, that Offer may be assigned implicitly as the contributor. On a Touchpoint linked to multiple Offers, the author must select one or more contributing Offers for a bottom-up selection; the system never guesses among them. The operation expands only those Offers; it must not silently expand every linked Offer. The same effective Job, ordinary Desired Outcome, Emotional or Social Job, or Financial Desired Outcome leaf may have multiple contributing Offer paths. Its visible projection is deduplicated and remains while at least one selected valid path remains, although attribution is retained explicitly for every semantic leaf and Offer path.

Two authoring directions produce the same durable intent structure:

1. **Top-down authoring:** follow `Product → Offer → Touchpoint → Child Touchpoint`. Decomposition is explicit: selecting upstream scope never silently selects it downstream, and each descendant may choose an independent subset bounded by its immediate owner.
2. **Bottom-up authoring:** follow `Child Touchpoint → Parent Touchpoint(s) → contributing Offer → Product`. The operation performs an atomic monotonic union of required Job membership and selected Desired Outcomes at every level; it never replaces broader upstream scope. Every Touchpoint stores its own durable contributor attribution. Each ancestor contributor must already have its own Offer relationship: the command never creates or inherits a structural Offer relation. One valid contributor may be inferred, but multiple valid paths require explicit resolution and are never guessed or expanded as a group. No valid path produces an unresolved/invalid atomic operation. Financial Desired Outcome propagation stops at each contributing Offer and never creates Product Job Intent.

Local removal is non-propagating: removing a Job or Desired Outcome from a Touchpoint changes only that local path. Upstream narrowing remains a separate explicit, impact-reviewed operation.

Bottom-up authoring documents upstream business intent; it is not temporary derived inheritance. Once the complete atomic semantic command succeeds, every created Product Job Intent, Offer selection, and Touchpoint selection is an independently authored record. Top-down and bottom-up authoring therefore converge on the same durable structure. Ambiguous or invalid contributor planning and any failed validation commit nothing, run no semantic cleanup, and leave all upstream and Touchpoint records unchanged. The exact transaction and persistence mechanism remains unresolved pending an accepted architecture decision.
...
The first edge is the existing Client ontology relationship. The second is a deduplicated cross-side projection from Desired Outcome to Touchpoint. A non-empty local Desired Outcome subset is required for this route; Job-only membership remains valid intent knowledge. A direct `CFJ/RJ/CCJ → Touchpoint` route is invalid and must not be rendered as fallback.
...
- deleting a Touchpoint or any local Touchpoint intent, including one of its local selections, deletes no upstream Product Job Intent, Offer selection or Financial Desired Outcome intent, Client-side Job, Desired Outcome, or Client-side relationship;
...
- unlinking an Offer from a Touchpoint when it has durable attributed paths requires an impact review before those paths are deleted;
...
In the default map, a satellite primarily communicates semantic reach that is not already readable from an ordinary visible local edge. When projected, same-kind references for an owner are visible as one compact group on a single local orbit. The group uses the existing entity-kind label and concrete target count, and discloses the existing target titles within the available map viewport. Orbit placement is a deterministic rendering concern: it may choose a relatively free sector, but it does not move authored nodes. A satellite creates neither a React Flow edge nor a domain relationship or entity record.
...
Entity Inspector provides transient browser-like Back and Forward history for successful entity-to-entity navigation **within an Inspector research session**, not a history of every Map selection. A confirmed Map-owned selection change starts a new single-entity root history, including entry via Map context menu or double-click; reselecting the same entity does not reset an existing history. Inspector-owned entity-link navigation pushes an entry, and following a new link after Back truncates the old forward tail. Back/Forward traverses existing entries, updates the one shared workspace selection, skips entities absent from the current `MapDocument`, and does not author domain state. Empty directions are disabled. Switching Inspector → Map → Inspector without changing the Map selection preserves the full history and current index, even after pan/zoom on Map; the workspace switch itself neither resets history nor adds an entry. Returning to Map retains the currently inspected entity as the shared selection for spatial orientation. Confirmed Map selection changes are history boundaries only after the existing dirty-session guard accepts them; an Inspector-owned new-entity creation starts its own coherent history root. The current runtime implements these boundaries through `selectFromMap()`, `performInspectorNavigation()`, and `performWorkspaceTransition()` in `MapSpike.tsx` and the transient history model in `inspector-navigation.ts`. The current Product Inspector's transitional section-wide draft uses its existing Apply, Discard, or Keep editing guard when navigation would abandon that draft; local operations under the target interaction model need a guard only while genuinely incomplete or awaiting destructive-impact confirmation. Inspector Back/Forward is distinct from nested editor `Back`, browser-native history, and future document undo. Browser and operating-system shortcuts for Inspector history remain unassigned until a platform audit establishes that they do not conflict with browser navigation or controls.
...
Entity Inspector may be activated through its workspace tab, through `Open in Entity Inspector` in an entity context menu, or through the global workspace toggle. The approved bidirectional `Map ↔ Entity Inspector` shortcut is `Ctrl+Shift+Space` on Windows/Linux and `Cmd+Shift+Space` on macOS. A stale or dismissible context menu or tooltip is dismissed before the shortcut switches the current selection between the peer workspaces. Editable controls retain native keyboard ownership; inline edits and semantically incomplete creation operations ignore the shortcut; the current Product Inspector's transitional section-wide draft retains its existing apply/discard confirmation; and active destructive-impact confirmations cannot be bypassed. The shortcut changes neither selection nor document state.
...
- **Unanchored entity birth → nearest free local niche.** Existing authored placements remain fixed when a new entity has no spatially relevant relation to an already placed entity. The search begins at the geometric center of the occupied bounds calculated from actual rendered node rectangles, evaluates a finite clockwise-ordered set of candidates in deterministic outward rings, and forbids physical node overlap. A modest soft guard around existing nodes prefers comfortable separation but is not an exclusion zone, and the nearest ring containing a physically valid candidate always wins. Empty maps use one canonical seed. Placement uses map coordinates only; viewport, camera, pan, and zoom do not influence it. This is not global auto-layout.

- **Title and body double-clicks have separate owners.** Double-clicking the authored entity's Title hit target starts inline Title editing; double-clicking its body opens Entity Inspector for deep editing. The editor owns a local draft while active, so ordinary typing preserves native caret and selection behavior without mutating the shared `MapDocument`. Enter, blur, and Tab commit the same valid single-line value (Tab retains native focus traversal); Escape discards it; a blank exit remains in editing; and closing returns focus predictably. Titles wrap within a role-sized node and are clamped to a bounded line count without changing its diameter. Hovering the Title or focusing it with the keyboard discloses the exact full Title in an overlay that flips and clamps within the Map panel.
...
- **Arrow and numpad direction → spatial focus.** From a focused authored node, Arrow keys address the North/East/South/West sectors and physical numpad 1–9 except 5 address all eight compass sectors, independent of Num Lock labels. Candidate geometry uses authored node centers in map coordinates and no React Flow internal state. Eight non-overlapping 45° sectors are centered on the compass directions; each exact 22.5° boundary belongs to the clockwise sector, so a genuine northwest candidate belongs to Northwest rather than North. Only candidates in the requested sector are considered, ordered by Euclidean distance and then stable entity ID. An empty sector is a no-op: it does not widen, wrap, move a node, or change focus. A selected target receives DOM focus and is conservatively revealed under the existing zoom-preserving reveal contract (including its already permitted minimum zoom-out). Editors, menus, Relations mode, and movement interaction retain keyboard ownership.
- **M → Move mode for the focused authored node.** Move mode is transient Map interaction state and delegates every accepted key to the existing authored-view Placement mutation. Arrow keys move by one 24-map-unit cardinal step; physical numpad 1–9 except 5 use the same 24-unit component on each addressed axis, so diagonal movement is deterministically longer by a factor of √2. Each key completes one Placement update. This step is a reversible view detail chosen to be smaller than the existing 40-unit duplicate offset and may be revised without changing domain meaning. Keyboard movement, like unconstrained manual drag, permits physical overlap: it does not invoke free-niche placement or imply pinning. Pan, zoom, entities, relationships, other placements, and epistemic state are unchanged.

| Move state | Owner | Entry / movement | Escape / boundary | Visible result |
| --- | --- | --- | --- | --- |
| `inactive` | Map keyboard arbiter | `M` from the focused authored node enters `moving(entityId)`; controls, menus, inline editors, Relations mode, and non-Map workspaces retain ownership | Existing Map behavior | Ordinary directional keys perform spatial focus navigation |
| `moving(entityId)` | Map interaction state, using the domain `movePlacement` operation | Arrow and physical numpad direction keys update only that entity's Placement in the current view | Escape exits without reverting completed moves; selection loss, entity removal, or workspace transition exits safely | Restrained node outline and mode indicator; directional keys move rather than navigate |
- **R → Relations mode** only when an eligible authored node is focused. Relations mode is transient UI interaction state; the map adapter supplies its deterministic relation groups, concrete target IDs, and exact authored/derived physical edge IDs. It neither authors relationships nor infers transitive ones. Left/Right wrap across groups; Up/Down enter and then remain clamped within the current concrete-target list. Enter on a concrete target selects and conservatively reveals that actual Map node and exits Relations mode, while the universal workspace shortcut instead selects the target and opens its Entity Inspector. Thus ordinary Enter retains its sibling meaning outside Relations mode.

| Relations state | Owner | Entry / movement | Enter | Escape | Visible result |
| --- | --- | --- | --- | --- | --- |
| `inactive` | Map interaction state | `R` from a focused eligible node chooses the adapter's first deterministic group; no target leaves state unchanged | Existing sibling behavior | Existing Map dismissal behavior | Ordinary authored map and satellite groups |
| `group` | Relations-mode reducer | Left/Right wrap through groups; Up/Down enter the concrete list at its first target | No target to follow | Restore focus to the exact source node and exit | Focused satellite group; no edge dimming yet |
| `concrete-list` | Relations-mode reducer | Up/Down clamp at list boundaries; Left/Right wrap groups and return to group focus | Select and reveal the actual target on Map, then exit; never opens Inspector | Return one level to `group` | Target overlay; unrelated existing edges are visually dimmed while adapter-provided path edges remain readable |

Relations highlighting is non-transitive and creates no renderer edges. In particular, a Financial Desired Outcome projection terminates at its applicable Offer or Touchpoint and is never extended to Product. Satellite focus does not create a second domain selection, and `S` and `M` remain available to their existing keyboard owners.
...
- **upward through Business hierarchy, required scope propagates automatically:** authoring at a Child Touchpoint passes through every Parent Touchpoint, with separate contributor attribution at each Touchpoint, before reaching the contributing Offer and Product; FDO stops at Offer;
...
Inspector should expose Client intent in its semantic hierarchy rather than force the user to think in internal record types. Touchpoint Inspector reuses semantic mini-dendrites from the existing Client ontology. DO-bearing Jobs should be shown as expandable mini-dendrites with their Desired Outcomes and can be selected only through Desired Outcome leaves. Selecting a DO includes the owning Job. Emotional/Social Jobs have no DO selector. FDO propagates only across Offer and Touchpoint scope, never Product scope. Client-side Inspectors may initiate the same Business-authoring operation as Business-side Inspectors; this is a different perspective on the same domain state, not a second relationship model. A semantic authoring command plans and validates before its single atomic durable commit. A transient picker or editor may collect a genuinely missing semantic choice and may close independently from commit; no section-wide Apply is required once the local operation is complete. Unresolved, invalid, cancelled, or failed commands mutate nothing.

Reusable Inspector presentation and interaction details are governed by [Inspector presentation contract](docs/inspector-presentation-contract.md) and [Inspector interaction contract](docs/inspector-interaction-contract.md). Under `AGENTS.md`, these must be reviewed during task planning before implementing another Inspector or changing an existing shared pattern; if accepted patterns change, keep the repository contracts and relevant regression tests in sync. This documentation requirement does not mean all Inspector kinds already match the Touchpoint runtime.

The hardened Touchpoint reference presents **Neighborhood** as derived/read-only groups with counts and progressive disclosure; no edit affordance appears for derived data. **Client scope** read state groups non-empty intent by the six canonical Client kinds, retains Job → Desired Outcome and Financial Desired Outcome structure, allows independent panel expansion, and places expanded panels first in its responsive layout. These are current Touchpoint read-state behaviors; their reusable principle is progressive disclosure without copying the same panel composition into every future Inspector. Its edit state is a distinct semantic discovery flow, not a second durable model.

Touchpoint Client scope edits inline as a state of the permanent **Client scope** document section, not as a separate nested editor. One ontology-aware **Search Client intent** field is immediately available near the top of edit mode; there is no permanent Entity kind selector. Title matches and temporary kind shortcuts share that search interaction, and a selected shortcut can return to the original query results. Immediate upstream semantic sources use initially collapsed progressive disclosures: an ordinary Touchpoint discloses each non-empty linked Offer, while a Child Touchpoint discloses its Parent as the sole semantic source and retains the Child's linked Offers only as contributor-attribution candidates. These owner-aware mini-dendrites preserve Job → Desired Outcome branches, direct Emotional/Social Jobs, and Financial Desired Outcomes; Parent provenance never fabricates, inherits, or implies a Child Offer relation or contributor.

The search and source disclosures reuse the existing bottom-up authoring operation. Full-path checkboxes commit safe local additions and removals immediately while the section remains in edit mode. Current or ancestor contributors are requested only when a real ambiguity exists, ambiguous ancestors resolve sequentially from domain-command results, and contributors are never guessed. Missing contributor paths remain inline invalid results. Destructive local impact receives an exact confirmation before commit; cancellation, unresolved choices, validation failure, and invalid ancestry leave the durable document and unrelated draft fields unchanged, while a complete command commits atomically.
...
| Select Desired Outcome on Touchpoint | Outcome has owning CFJ/RJ/CCJ; every Touchpoint ancestor has a valid linked contributor path | Explicit contributor at the authored Touchpoint and each ancestor; a sole ancestor candidate may be inferred | Create/extend owning Job’s Product Job Intent if missing | Select/extend intent only in chosen Offers | Select owning Job and outcome per contributor atomically | `Job → Desired Outcome → Touchpoint`; no direct Job route | Confirm upstream additions in operation preview | Atomic upstream completion, not deletion | Existing Job, outcome, owning relationship, and unrelated intent | Reject orphan/wrong-owner outcome or invalid Job; ambiguity remains unresolved and no path is invalid, with no mutation |
| Select Emotional/Social Job bottom-up with no Product intent | Touchpoint and every ancestor have valid linked Offer paths; EJ/SJ exists | Explicit authored contributor plus one resolved contributor per ancestor | Create Product Job Intent for each distinct contributing Product | Create Offer selection in chosen Offers | Create attributed Job selections | One deduplicated direct `EJ/SJ → Touchpoint` route | Confirm upstream additions | Atomic creation across owners | Job and Client-side relationships | Ambiguity remains unresolved; invalid path or failure leaves all owners unchanged |
| Select DO-bearing Job bottom-up with no Product intent | CFJ/RJ/CCJ exists, ≥1 owning DO is selected, and every ancestor has a valid path | Authored contributors plus one resolved contributor per ancestor | Create/extend Product Job Intent with selected/created DO(s) | Create Offer selection in chosen Offers | Create attributed Job/DO selections | `Job → DO → Touchpoint` only | Yes; UI must select/create ≥1 DO and confirm upstream additions | Atomic creation across owners | Job, existing DOs, owning/context relationships | No direct Job fallback; unresolved ambiguity or missing DO commits nothing |
| Select Desired Outcome bottom-up with no Product intent | Outcome and owning CFJ/RJ/CCJ exist; Touchpoint and every ancestor path are valid | Authored contributors plus one resolved contributor per ancestor | Create Job intent with selected outcome for each distinct Product | Create Offer selection in chosen Offers | Create attributed Job/outcome selections | One deduplicated outcome route; no direct Job route | Confirm upstream additions | Atomic creation across owners | Job, outcome, owning relationship, unrelated intent | Roll back all new records on orphan outcome, invalid contributor, or failure |
| Select intent on multi-Offer Touchpoint | Two or more valid Offers exist at the authored Touchpoint or an ancestor | Authored Touchpoint keeps its explicit subset; each ambiguous ancestor requires one explicit contributor, never first/all implicitly | Create/extend only Product intents required by chosen paths | Create/extend only chosen Offer selections | Create one attributed selection per chosen path | Deduplicate equal effective routes | Yes, contributor picker/preview | Atomic only across chosen paths | Unchosen Offer scopes and existing selections | Reject no contributor, unlinked Offer, or incompatible Product path |
| Select FDO on Touchpoint | FDO exists; the Touchpoint and every ancestor have a linked Offer path | Authored contributors plus one resolved linked Offer per ancestor | None | Add FDO to each chosen Offer Financial Intent if missing | Create attributed FDO selections | One deduplicated `FDO → Touchpoint` route | Confirm upstream Offer additions | Atomic Offer + Touchpoint completion | Products, Product Job Intents, and all Client-side records | Unresolved ambiguity or invalid/unlinked ancestor path commits nothing |
...
After a successful complete relation-authoring commit, the command-owned existing entity may reconsider its placement only when its represented map-anchor set changes (including gaining its first represented relation). The deterministic affected entity is the entity being edited by the active command: an Offer when its Product anchor changes; a Touchpoint when its Offer, parent-Touchpoint, mitigation, or attributed Job/Desired Outcome/Financial Desired Outcome route changes; and the edited Client entity or Repulsor when its represented contextual, ownership, resistance, or mitigation anchor set changes. Product- or Offer-intent edits that create no represented map edge do not trigger movement merely because draft records changed. Cancelled, rejected, and failed drafts never trigger reconsideration.

Reconsideration compares the current placement with a fixed set of nearby map-coordinate candidates. It minimizes aggregate relation distance before crossing count, edges through the node, and soft spacing, forbids physical overlap, and requires an explicit minimum relation-distance benefit before moving. Consequently a nearby candidate with one crossing is preferred to an absurdly remote zero-crossing candidate. Only the command-owned affected placement may change; anchors and neighbors remain fixed, and there is no global relaxation. Pan, zoom, viewport size, and camera state do not participate. Manual drag and keyboard movement remain ordinary authored placement rather than a permanent pin, so a previously moved node remains eligible under the same accepted post-commit triggers.

An Entity Inspector operation that creates the currently supported mandatory Business prerequisite chain is one atomic **birth batch**. Its geometry is deterministic sequential self-construction in dependency order (Product, then Offer, then Touchpoint), not simultaneous or global optimization: an unanchored new Product uses the same free-niche search as single-node birth, and each subsequent Offer or Touchpoint uses the same relation-anchored solver around its prerequisite. After every provisional placement, the new node and its required relationship participate in the next calculation, so later nodes avoid both existing and earlier batch geometry and score provisional edges. This topology order imposes no compass direction; right-first remains only an equal-quality tie-break. Existing authored nodes never move, physical overlap with them or between batch nodes is forbidden, and the complete document is committed once. This contract covers the current Product → Offer → Touchpoint prerequisite forms, not arbitrary future multi-entity graph operations; global layout and force behavior remain out of scope.

After Map-side creation, the camera conservatively reveals the new node and its immediate relation neighborhood. It does nothing when that local structure is already inside a safe visible inset, pans minimally while preserving the current zoom when it fits, and zooms out only as much as needed when it does not; it does not automatically zoom in or fit the whole graph. Inspector-side creation leaves the hidden camera alone until Map activation, when the selected entity and, for a Business birth batch, its local created fragment including an existing external attachment anchor receive the same conservative reveal. Camera response occurs after placement and cannot affect placement coordinates. Context menus and contextual creation editors anchor to their actual entity or pointer interaction source and independently flip and clamp into the visible Map panel; overlay positioning does not predict or determine the new node position. Manual dragging remains an authored local override and does not use this collision rule.
