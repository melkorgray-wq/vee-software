...
Touchpoint Client scope is upstream-first: its durable read mini-dendrites remain visible while editing, followed by separate Offer sources and, when it has durable Client scope, a Parent Touchpoint source. Each source preserves owner-aware Job → Desired Outcome branches, direct Emotional/Social Jobs, and Financial Desired Outcomes. Offer blocks identify their Offer as the prospective contributor. A Parent block supplies semantic scope and provenance only: the Child resolves attribution independently through its own linked Offers, which need not include the Offer that contributed the Parent path. Full-path checkboxes commit safe local additions and removals immediately; per-source and all-Offer **Select all** commands are additive and preserve independent paths through different Offers. Parent scope never fabricates, inherits, or implies an Offer relation.

The secondary **Find Client intent elsewhere** disclosure is bottom-up discovery, with hierarchical title search and exact kind browsing. It asks for current or ancestor contributors only when a real ambiguity exists, resolves ambiguous ancestors sequentially from domain-command results, and never guesses contributors. Missing contributor paths remain inline invalid results. Destructive local impact receives an exact confirmation before commit; cancellation, unresolved choices, validation failure, and invalid ancestry leave the durable document and unrelated draft fields unchanged, while a complete command commits atomically.
