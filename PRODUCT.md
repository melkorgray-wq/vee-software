...
The current runtime still permits a direct Job → Touchpoint fallback when a Touchpoint Job selection has an empty Desired Outcome scope. The accepted product contract below now supersedes that behavior for Core Functional Job, Related Job, and Consumption Chain Job: these are DO-bearing Jobs and may materialize cross-side only through one or more ordinary Desired Outcomes. Emotional Job and Social Job remain direct Job → Touchpoint routes because ordinary Desired Outcome does not exist for those kinds in the accepted ontology. Financial Desired Outcome remains an Offer-owned outcome-level intent that routes directly to Touchpoint. Bringing runtime validation/rendering into conformance is a required next implementation correction, not a new ontology decision.
