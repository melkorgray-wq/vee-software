...
Inspector scales relationship authoring by keeping the ordinary **Connected** section limited to durable connections. The disconnected ontology is disclosed only in an explicit **Add connection** picker, where title search, exact entity-kind browsing, and Job → Desired Outcome context support selection without flattening the Client hierarchy. Cancel closes that transient picker without changing the draft or durable document; a selected candidate joins Connected only after the owning Inspector transaction is applied. Empty and no-results states remain explicit. Connected entity titles retain Inspector navigation behavior.
