...
Touchpoint Client scope is upstream-first and edits inline as a state of the permanent **Client scope** document section, not as a separate nested editor. The same owner-aware mini-dendrites combine durable and available intent in place while preserving Job → Desired Outcome branches, direct Emotional/Social Jobs, and Financial Desired Outcomes. Contributor and provenance variants remain compactly attached to their semantic element; empty Offer groupings are omitted. A Parent Touchpoint supplies semantic scope and provenance only: the Child resolves attribution independently through its own linked Offers, which need not include the Offer that contributed the Parent path. Full-path checkboxes commit safe local additions and removals immediately while the section remains in edit mode. Parent scope never fabricates, inherits, or implies an Offer relation.
