...
Entity Inspector editing follows semantic completeness rather than a blanket save-on-control-change or blanket draft-and-Apply rule. A semantically complete, unambiguous, valid, non-destructive action commits automatically as exactly one owner-level domain transaction. Ambiguous operations remain transient until the missing semantic choice is supplied. Destructive operations remain staged until their disclosed impact is explicitly confirmed, after which the complete change commits once. Validation or transaction failure changes no durable record and leaves a coherent interaction that can be corrected or retried.
...
`Ctrl/Cmd+S` is also not implemented as persistence. If the active editor contains a complete valid staged owner operation, the proposed command may commit that operation to the in-memory document. If no such operation is pending, its message should state **“Changes are already live in this session.”** It must not claim external persistence. An incomplete or invalid draft remains uncommitted and receives its existing validation guidance rather than being inserted into transaction history.
...
| --- | --- | --- | --- |
| `complete` | The active Product, Offer, or Touchpoint editor has every required semantic input and contributor, validation succeeds, and no confirmation is required | One atomic owner-level domain operation commits automatically | The committed relationship or property is shown; no redundant Apply affordance is presented for that action |
...
Product Client-intent branch construction, Product property edits, Offer intent and property edits, and ordinary Touchpoint properties (Title, linked Offers, parent, Located in, URL, and mitigation) remain staged because each form can represent an incomplete multi-control owner operation. Destructive narrowing or removal in those editors also remains staged for impact confirmation. A complete Touchpoint semantic-leaf addition is different: one linked Offer is an unambiguous implicit contributor and commits immediately; with multiple linked Offers it remains incomplete until the author chooses contributor(s), then the whole upstream Product/Offer completion and attributed Touchpoint selection commit atomically. Touchpoint Offer unlink remains staged and destructive whenever attributed paths are affected. No checkbox performs an independent durable write merely because its value changed.

Product Inspector Client-intent authoring is a transactional draft until **Apply**. Product properties, selections, and Client entities created there are not durable and receive no map placement before Apply. A staged Client entity is committed only while it still participates in that Product draft; unselecting a staged Job discards its staged-only descendants, and unselecting a staged Desired Outcome discards that outcome. Existing durable Client entities are never deleted by unselecting Product intent.

Disclosure of a DO-bearing Job is independent from Product-intent selection. Selecting a Desired Outcome always includes its owning Job, but selecting a Job never assumes any Desired Outcome, and removing the final selected Desired Outcome leaves a valid, incomplete upstream Product Job Intent. Core Functional Job, Related Job, and Consumption Chain Job use ordinary Desired Outcomes; Emotional and Social Jobs do not. An ordinary Desired Outcome remains owned by its Job through the Client-side `Job → Desired Outcome` relationship and never becomes a standalone Product goal. Creating one during intent authoring creates the Client-side entity and its ordinary owning relationship before selecting it inside Product Job Intent; “before” describes mutation order within the same atomic Apply, not an earlier durable commit. It creates no direct Product → Desired Outcome relationship.

Product Inspector Apply constructs and validates the complete next document before committing it once. Failure commits nothing and preserves the recoverable draft. During one edit session, durable Job branches remain in the section established when the draft opened; temporarily deselecting a durable Job remembers its latest draft Desired Outcome scope for re-selection until Apply or Discard. An in-app guard protects a dirty Product draft only when an action would replace or abandon it, so switching between Map and Entity Inspector for the same Product preserves the draft without prompting. Removing or narrowing durable Product intent requires a consolidated in-app downstream-impact review before commit when Offer or Touchpoint selections would be removed or a Touchpoint Desired Outcome subset would be narrowed; additive changes require no such review.
...
Offer Inspector uses the same Job → Desired Outcome mini-dendrite language as Product Inspector. A Job checkbox controls local Offer membership, while nested Desired Outcome checkboxes control the Offer-owned subset within Product-authored availability; an empty subset remains valid. Financial Desired Outcome intent remains a separate Offer-owned selection. Changing Linked Product does not initialize intent from the new Product. During an edit session, Offer-intent branches remain in the sections established when the draft opened until Apply or Discard; destructive intent removal or Product change requires an in-app downstream-impact review when attributed Touchpoint paths would be removed.
...
Entity Inspector provides transient browser-like Back and Forward history for successful entity-to-entity navigation within Inspector. Connected entity titles open their target Inspector directly without authoring or changing a relationship. Traversal updates the one shared workspace selection, skips entities that are no longer present in the current `MapDocument`, never persists history as domain state, and clears the forward branch after a new navigation. Empty directions are disabled. Inspector navigation that would abandon a dirty draft uses the same Apply, Discard, or Keep editing guard as other selection changes. Browser and operating-system shortcuts remain unassigned until a platform audit establishes that they do not conflict with browser navigation or controls.
