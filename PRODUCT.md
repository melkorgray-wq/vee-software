...
Touchpoint Client scope is upstream-first: its durable read mini-dendrites remain visible while editing, followed by separate Offer sources and, when it has durable Client scope, a Parent Touchpoint source. Each source preserves owner-aware Job → Desired Outcome branches, direct Emotional/Social Jobs, direct Offer-owned Financial Desired Outcomes, and the actual contributing Offer. Full-path checkboxes commit safe local additions and removals immediately; per-source and all-Offer **Select all** commands are additive and preserve independent paths through different Offers. Parent scope never fabricates or implies an Offer relation: a Parent path is selectable for a Child only through an Offer already linked to that Child.
