...
When a displayed Inspector value already has its own action, the value and its edit affordance are distinct semantic surfaces. Entity names may navigate internally and URLs may navigate externally, while the pencil remains the explicit trigger for local editing. A single hit area must not conceal a combination of navigation and editing.
...
Entity Inspector provides transient browser-like Back and Forward history for successful entity-to-entity navigation within Inspector. Connected entity titles open their target Inspector directly without authoring or changing a relationship. Traversal updates the one shared workspace selection, skips entities that are no longer present in the current `MapDocument`, never persists history as domain state, and clears the forward branch after a new navigation. Empty directions are disabled. The current Product Inspector's transitional section-wide draft uses its existing Apply, Discard, or Keep editing guard when navigation would abandon that draft; local operations under the target interaction model need a guard only while genuinely incomplete or awaiting destructive-impact confirmation. Browser and operating-system shortcuts remain unassigned until a platform audit establishes that they do not conflict with browser navigation or controls.
