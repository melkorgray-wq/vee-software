...
Offer Inspector uses the same Job → Desired Outcome mini-dendrite language as Product Inspector, but its Job-level selection carries each Product Job Intent as a whole branch. Product-authored ordinary Desired Outcomes are disclosed as read-only scope and cannot be narrowed there; an incomplete DO-bearing Product intent may still be selected upstream. Financial Desired Outcome intent remains a separate Offer-owned selection. Changing Linked Product does not initialize intent from the new Product. During an edit session, Offer-intent branches remain in the sections established when the draft opened until Apply or Discard; destructive intent removal or Product change requires an in-app downstream-impact review when attributed Touchpoint paths would be removed.
...
For a DO-bearing Job, Touchpoint authoring must select one or more ordinary Desired Outcome leaves from the allowed upstream scope; the Job can be selected only through those leaves. If the upstream Product intent is still empty/incomplete, the UI must ask the user to create or select at least one Desired Outcome before the Touchpoint connection can become effective. The system must not create a direct fallback route around that missing layer.
...
1. **Top-down authoring:** follow `Product → Offer → Touchpoint → Child Touchpoint`. Descendant scope propagates only through an explicit subset choice; existing descendants never acquire new semantic scope implicitly. If the Job is CFJ/RJ/CCJ, every selected Touchpoint path must include at least one ordinary Desired Outcome.
2. **Bottom-up authoring:** follow `Child Touchpoint → Parent Touchpoint(s) → contributing Offer → Product`. Required scope passes through every structural Touchpoint ancestor, and every Touchpoint stores its own durable contributor attribution. Each ancestor contributor must already have its own Offer relationship: the command never creates or inherits a structural Offer relation. One valid contributor may be inferred, but multiple valid paths require explicit resolution and are never guessed or expanded as a group. No valid path produces an unresolved/invalid atomic operation. Financial Desired Outcome propagation stops at each contributing Offer and never creates Product Job Intent.
