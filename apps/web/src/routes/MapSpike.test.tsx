...
describe('map-first authoring interactions', () => {
  ...
  it('offers optional location and URL context while creating a Touchpoint root', async () => {
    ...
    const location = screen.getByRole('group', { name: 'Where does this Touchpoint exist?' });
    expect(location).toBeInTheDocument(); expect(within(location).getByLabelText('No location selected')).toBeChecked(); expect(screen.getByLabelText(/URL/)).not.toBeRequired();
    ...
    await user.click(screen.getByRole('button', { name: 'Add element' })); await user.selectOptions(screen.getByLabelText('Business element type'), 'touchpoint'); await user.type(screen.getByLabelText('Title'), 'Service page'); await user.selectOptions(screen.getByLabelText('Existing Offer'), screen.getByRole('option', { name: 'Subscription' })); await user.click(screen.getByLabelText('Create new location'));
    expect(screen.getByLabelText('New location title')).toBeRequired(); await user.type(screen.getByLabelText('New location title'), 'Website'); await user.type(screen.getByLabelText(/URL/), 'https://example.test/service');
  ...
  it('discards drafted locations on switching, cancellation, and prerequisite validation failure', async () => {
    const user = userEvent.setup(); render(<MapSpike />); await globalProduct(user); await quickOffer(user); await openInspector(user);
    await user.click(screen.getByRole('button', { name: 'Add element' })); await user.selectOptions(screen.getByLabelText('Business element type'), 'touchpoint'); await user.type(screen.getByLabelText('Title'), 'Cancelled point'); await user.selectOptions(screen.getByLabelText('Existing Offer'), screen.getByRole('option', { name: 'Subscription' })); await user.click(screen.getByLabelText('Create new location')); await user.type(screen.getByLabelText('New location title'), 'Abandoned location'); await user.click(screen.getByRole('button', { name: 'Cancel' }));
    expect(screen.queryByRole('button', { name: 'Cancelled point' })).not.toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: 'Add element' })); await user.selectOptions(screen.getByLabelText('Business element type'), 'touchpoint'); await user.type(screen.getByLabelText('Title'), 'Cleared point'); await user.selectOptions(screen.getByLabelText('Existing Offer'), screen.getByRole('option', { name: 'Subscription' })); await user.click(screen.getByLabelText('Create new location')); await user.type(screen.getByLabelText('New location title'), 'Stale location'); await user.click(screen.getByLabelText('No location selected')); await user.click(screen.getByRole('button', { name: 'Create' }));
    expect(screen.getByRole('heading', { name: 'Cleared point' })).toBeInTheDocument(); expect(screen.getByLabelText('Located in')).toHaveValue('');

    await user.click(screen.getByRole('button', { name: 'Add element' })); await user.selectOptions(screen.getByLabelText('Business element type'), 'touchpoint'); await user.type(screen.getByLabelText('Title'), 'Invalid chain'); await user.click(screen.getByLabelText('Create new Offer')); await user.type(screen.getByLabelText('New Offer title'), 'Uncommitted offer'); await user.click(screen.getByLabelText('Create new Product')); await user.type(screen.getByLabelText('New Product title'), 'Uncommitted product'); await user.click(screen.getByLabelText('Create new location')); await user.type(screen.getByLabelText('New location title'), 'Uncommitted location'); await user.clear(screen.getByLabelText('New Offer title')); await user.click(screen.getByRole('button', { name: 'Create' }));
    ...
    for (const name of ['Invalid chain', 'Uncommitted offer', 'Uncommitted product']) expect(screen.queryByRole('button', { name })).not.toBeInTheDocument();
  });
