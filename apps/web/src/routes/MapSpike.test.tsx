...
describe('map-first authoring interactions', () => {
  ...
  it('authors Product Jobs with nested Desired Outcomes and preselects them in a new Offer draft', async () => {
    ...
    const jobs = screen.getByRole('group', { name: 'Which Client Jobs does this Product intend to address?' }); expect(within(jobs).getByLabelText(/Make progress/)).toBeChecked(); await user.click(within(jobs).getByRole('button', { name: 'Add Desired Outcome' })); expect(within(jobs).getByLabelText('Finish faster')).toBeChecked();
    ...
    await user.click(screen.getByRole('button', { name: 'Orbit' })); fireEvent.keyDown(window, { key: 'Tab' }); const offer = contextualEditor('Add Offer'); expect(within(offer.getByRole('group', { name: 'Which parts of this Product’s Client intent does this Offer carry?' })).getByLabelText('Make progress')).toBeChecked(); prompt.mockRestore();
  });
