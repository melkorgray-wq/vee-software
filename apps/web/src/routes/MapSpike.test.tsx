...
describe('map-first authoring interactions', () => {
  ...
  it('authors a missing Core Functional Job intent bottom-up through the linked Offer', async () => {
    const user = userEvent.setup(); render(<MapSpike />);
    await user.click(screen.getByRole('button', { name: 'Add element' })); await user.click(screen.getByRole('button', { name: 'Client side' })); await user.type(screen.getByLabelText('Title'), 'Make progress'); await user.click(screen.getByRole('button', { name: 'Create element' }));
    ...
    const touchpointId = screen.getByRole('button', { name: 'Checkout' }).getAttribute('data-node-id'); const inspector = within(screen.getByRole('complementary'));
    ...
    await user.click(editor.getByLabelText(/Make progress/)); expect(editor.getByLabelText('Subscription')).toBeChecked(); expect(within(editor.getByRole('region', { name: 'Change preview' })).getByText('A new Product Job Intent will be created.')).toBeInTheDocument();
    await user.click(editor.getByRole('button', { name: 'Confirm client intent' })); expect(screen.getByRole('status')).toHaveTextContent('Client intent added');
    expect(within(inspector.getByRole('group', { name: 'Touchpoint intent scope' })).getByLabelText('Make progress')).toBeChecked(); expect(document.querySelector(`[data-source="${jobId}"][data-target="${touchpointId}"]`)).toBeInTheDocument();
  });
