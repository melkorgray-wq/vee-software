...
describe('map-first authoring interactions', () => {
  ...
  it('rejects incomplete Core Functional Job intent at the Touchpoint boundary', async () => {
    ...
    await user.click(editor.getByRole('button', { name: 'Confirm client intent' })); expect(editor.getByRole('alert')).toHaveTextContent('requires at least one Desired Outcome');
    expect(within(inspector.getByRole('group', { name: 'Touchpoint intent scope' })).queryByLabelText('Make progress')).not.toBeInTheDocument(); expect(document.querySelector(`[data-source="${jobId}"][data-target="${touchpointId}"]`)).not.toBeInTheDocument();
  });
