...
describe('map-first authoring interactions', () => {
  ...
  it('keeps context-menu and Shift+Tab Repulsor targets aligned across Client roots', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Save' })); const financialShortcut = new KeyboardEvent('keydown', { key: 'Tab', shiftKey: true, cancelable: true }); fireEvent(window, financialShortcut); expect(financialShortcut.defaultPrevented).toBe(false); expect(screen.queryByRole('heading', { name: 'Add Repulsor' })).not.toBeInTheDocument();
    fireEvent.contextMenu(screen.getByRole('button', { name: 'Save' })); expect(screen.queryByRole('menuitem', { name: 'Add Repulsor' })).not.toBeInTheDocument();
  });
