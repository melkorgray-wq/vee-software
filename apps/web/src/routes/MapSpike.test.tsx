...
describe('map-first authoring interactions', () => {
  ...
  it('keeps Product Client-intent authoring in Inspector and preselects it in a new Offer draft', async () => {
    ...
    await openMap(user); await user.click(screen.getByRole('button', { name: 'Orbit' })); fireEvent.keyDown(window, { key: 'Tab' }); const offer = contextualEditor('Add Offer'); expect(within(offer.getByRole('group', { name: 'Which parts of this Product’s Client intent does this Offer carry?' })).getByLabelText('Make progress')).toBeChecked(); prompt.mockRestore();
  });
