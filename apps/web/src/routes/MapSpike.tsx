...
export function MapSpike({ initialDocument = INITIAL_DOCUMENT }: { initialDocument?: MapDocument } = {}) {
  ...
  function touchpointClientScopeSection() {
    ...
    return <section className={`touchpoint-client-scope${editing ? ' is-editing' : ''}`} aria-labelledby="touchpoint-client-scope-heading" onKeyDown={event => { if (event.key !== 'Escape' || !connectionPicker) return; event.preventDefault(); event.stopPropagation(); if (['current-contributor-choice', 'ancestor-contributor-choice', 'invalid'].includes(connectionPicker.mode)) cancelResolver(); else if (connectionPicker.mode !== 'upstream' || connectionPicker.query || connectionPicker.kind) resetTransientSearch(); else exitEdit(); }}>
      <div className="touchpoint-client-scope-heading"><h4 id="touchpoint-client-scope-heading">Client scope</h4><button ref={connectionPickerButtonRef} type="button" className={`business-structure-edit-value${editing ? ' touchpoint-client-scope-done' : ''}`} aria-label={editing ? 'Finish editing Client scope' : 'Edit Client scope'} aria-pressed={editing} disabled={!offers.length} onClick={() => editing ? exitEdit() : setConnectionPicker({ mode: 'upstream', query: '', kind: undefined, contributorOfferIds: [], ancestorContributingOfferIds: {} })}>{editing ? 'Done' : <span className="business-structure-edit-affordance" aria-hidden="true">✎</span>}</button></div>
      ...
      {connectionPicker && <div className="touchpoint-client-scope-content inline-intent-editor" aria-describedby="touchpoint-client-scope-immediate-help">
        <p id="touchpoint-client-scope-immediate-help" className="touchpoint-client-scope-help">Changes apply immediately</p>
